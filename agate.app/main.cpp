...
int main()
{
	...
	window = glfwCreateWindow(1280, 720, "Agate.App", NULL, NULL);
	if (!window)
	{
		glfwTerminate();
		return -1;
	...
	GLint vertexShader = Shader::IdFromFile("./assets/vertex_shader.vert");
	shader->SetVertexShader(vertexShader);

	GLint fragmentShader = Shader::IdFromFile("./assets/fragment_shader.frag");
